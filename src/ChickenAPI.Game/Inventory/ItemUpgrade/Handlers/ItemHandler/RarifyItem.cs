...
namespace ChickenAPI.Game.Inventory.ItemUpgrade.Handler
{
    public class RarifyItemHandler
    {
        ...
        public static void RarifyNpc(IPlayerEntity player, ItemUpgradeEventArgs e)
        {
            ...
            player.EmitEvent(new RarifyEventArgs
            {
                Item = e.Item,
                Mode = mode,
            ...
            player.SendPacket(new ShopEndPacket
            {
                PacketType = ShopEndPacketType.CloseWindow
            });
        }

        [ItemUpgradeHandler(UpgradePacketType.RarifyItemProtected)]
        public static void RarifyScroll(IPlayerEntity player, ItemUpgradeEventArgs e)
        {
            if (e.Item.Item.EquipmentSlot != EquipmentType.Armor && e.Item.Item.EquipmentSlot != EquipmentType.MainWeapon &&
                e.Item.Item.EquipmentSlot != EquipmentType.SecondaryWeapon)
            {
