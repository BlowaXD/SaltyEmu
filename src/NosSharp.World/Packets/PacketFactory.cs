...
namespace NosSharp.World.Packets
{
    public static class PacketFactory
    {
        ...
        public static APacket Deserialize(string packetContent, Type packetType, bool includesKeepAliveIdentity = false)
        {
            try
            {
                ...
                var deserializedPacket = packetType.CreateInstance() as APacket;
        ...
        public static void Initialize<TBaseType>() where TBaseType : APacket
        {
            if (IsInitialized)
            {
                return;
        ...
        public static string Serialize<TPacket>(TPacket packet) where TPacket : APacket
        {
            try
            {
        ...
        private static APacket Deserialize(string packetContent, APacket deserializedPacket, KeyValuePair<Tuple<Type, string>,
            ...
            bool includesKeepAliveIdentity)
        {
            ...
            if (shouldRemoveSeparator)
            {
                ...
                if (((splittedSubpacketParts.Count + (includesKeepAliveIdentity ? 1 : 0)) % subPacketTypePropertiesCount) != 0)
                {
        ...
        {
            foreach (ValidationAttribute s in validationAttributes)
            {
            ...
            if (packetPropertyType.BaseType?.Equals(typeof(Enum)) == true)
            {
                ...
                {
                    // log could not convert value to packetPropertyType
        ...
        where TPacketDefinition : APacket
        {
        ...
        private static void SetDeserializationInformations(APacket packetDefinition, string packetContent, string packetHeader)
        {
