...
namespace SaltyEmu.BasicPlugin.EventHandlers.Upgrading
{
    public class Upgrading_UpgradeEquipment_Handler : GenericEventPostProcessorBase<UpgradeEquipmentEvent>
    {
        ...
        protected override async Task Handle(UpgradeEquipmentEvent e, CancellationToken cancellation)
        {
            ...
            {
                await player.SendChatMessage("ITEM_IS_FIXED", SayColorType.Yellow);
            ...
            if (e.Item.Rarity == 8)
            {
                if (rnd < upfail[e.Item.Upgrade])
                {
                    ...
                    {
                        await player.SendChatMessage("UPGRADE_FAILED", SayColorType.Purple);
                    ...
                    else
                    {
                        ...
                        await player.SendChatMessage("SCROLL_PROTECT_USED", SayColorType.Purple);
                ...
                else
                {
                    ...
                    await player.SendChatMessage("UPGRADE_SUCCESS", SayColorType.Green);
