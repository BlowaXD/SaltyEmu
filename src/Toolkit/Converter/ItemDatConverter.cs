...
namespace Toolkit.Converter
{
    public class ItemDatConverter
    {
        ...
        {
            string path = inputDirectory + '/' + _file;
            ...
            foreach (string line in lines)
            {
                    ...
                    switch (item.ItemType)
                    {
                        ...
                        case ItemType.Jewelery:
                            switch (item.EquipmentSlot)
                            {
                                case EquipmentType.Amulet:
                                    item.LevelMinimum = Convert.ToByte(currentLine[2]);
                                    if (item.Id > 4055 && item.Id < 4061 || item.Id > 4172 && item.Id < 4176)
                                    {
                                        item.ItemValidTime = 10800;
                                    }
                                    else if (item.Id > 4045 && item.Id < 4056 || item.Id == 967 || item.Id == 968)
                                    {
                        ...
                        case ItemType.Special:
                            switch (item.Id)
                            {
                                ...
                                case 1903:
                                    item.Effect = 789;
                                    item.EffectValue = (int)item.Id + 2152;
                                    break;
                ...
                else if (currentLine.Length > 1 && currentLine[1] == "BUFF")
                {
                    for (int i = 0; i < 5; i++)
                    {
                        ...
                        {
                            RelationType = BCardRelationType.Item,
                            RelationId = item.Id,
                            Type = type,
            ...
            _itemService.Save(_items);
            _bCardService.Save(_itemCards);
        }
