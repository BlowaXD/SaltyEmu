...
namespace SaltyEmu.Communication.Communicators
{
    public abstract class MappedRepositoryMqtt<T> : MqttIpcClient<T>, IMappedRepository<T> where T : class, IMappedDto
    {
        ...
        public T Save(T obj)
        {
            return RequestAsync<RepositorySaveRequest<T>.Response>(new RepositorySaveRequest<T> { Objects = new T[] { obj } }).ConfigureAwait(false).GetAwaiter().GetResult().Objects.ElementAt(0);
        }
